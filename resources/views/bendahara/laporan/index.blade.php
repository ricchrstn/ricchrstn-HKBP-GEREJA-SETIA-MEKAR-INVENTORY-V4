...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
                ...
                <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-5 mb-0 bg-white rounded-t-2xl">
                        <div class="flex flex-wrap items-center justify-between">
                            <div>
                                <h6 class="mb-0 text-lg font-bold text-slate-700">Laporan Keuangan</h6>
                                <p class="mb-0 text-sm leading-normal text-slate-400">Generate laporan kas dan pengadaan barang</p>
                            </div>
        ...
        <div class="flex flex-wrap items-center justify-between px-6 pt-4 pb-2 bg-gray-50 rounded-t-xl">
            ...
            <form id="filterForm" method="GET" action="{{ route('bendahara.laporan.index') }}" class="flex items-center">
                <div class="flex flex-wrap items-end gap-4">
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Jenis Laporan</label>
                        <select name="jenis_laporan" id="jenisLaporan" class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500">
                            <option value="">Semua Laporan</option>
                            <option value="kas" {{ request('jenis_laporan') == 'kas' ? 'selected' : '' }}>Kas</option>
                            <option value="pengadaan" {{ request('jenis_laporan') == 'pengadaan' ? 'selected' : '' }}>Pengadaan</option>
                        </select>
                    </div>
                </div>
            </form>
                        ...
                        <div class="flex flex-wrap items-center justify-between">
                            <div>
                                ...
                                <p class="mb-0 text-sm text-slate-400 mt-1">
                                    ...
                                    <span class="font-medium text-slate-600">{{ $laporanData->total() }}</span> data ditemukan
                                </p>
                            </div>
                    ...
                    <div class="flex-auto px-0 pt-0 pb-2">
                        <div class="p-0 overflow-x-auto">
                            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                                <thead>
                                    <tr>
                                        <th class="px-6 py-3 text-center text-xxs font-bold uppercase text-slate-400">ID</th>
                                        <th class="px-6 py-3 text-left text-xxs font-bold uppercase text-slate-400">Jenis</th>
                                        ...
                                        <th class="px-6 py-3 text-left text-xxs font-bold uppercase text-slate-400">Status</th>
                                    ...
                                    @forelse ($laporanData as $data)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="p-2 align-middle">
                                                @if(isset($data->jenis))
                                                    @if($data->jenis == 'masuk')
                                                        <span class="bg-gradient-to-tl from-green-600 to-green-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Pemasukan</span>
                                                    @elseif($data->jenis == 'keluar')
                                                        <span class="bg-gradient-to-tl from-red-600 to-red-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Pengeluaran</span>
                                                    @endif
                                                @elseif(isset($data->status))
                                                    <span class="bg-gradient-to-tl from-blue-600 to-blue-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Pengadaan</span>
                                                @else
                                                    <span class="bg-gradient-to-tl from-gray-600 to-gray-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Kas</span>
                                                @endif
                                            </td>
                                            <td class="p-2 align-middle">
                                                ...
                                                @if(isset($data->kode_transaksi))
                                                    <p class="mb-0 text-xs text-slate-400">{{ $data->kode_transaksi }}</p>
                                                @elseif(isset($data->kode_pengajuan))
                                                    <p class="mb-0 text-xs text-slate-400">{{ $data->kode_pengajuan }}</p>
                                                @endif
                                            </td>
                                            <td class="p-2 align-middle">
                                                @if(isset($data->tanggal))
                                                    <span class="text-sm">{{ date('d/m/Y', strtotime($data->tanggal)) }}</span>
                                            ...
                                            <td class="p-2 align-middle text-center">
                                                <span class="text-sm font-semibold">
                                                    ...
                                                    @elseif(isset($data->estimasi_harga))
                                                        {{ number_format($data->estimasi_harga, 0, ',', '.') }}
                                                    @else
                                                        N/A
                                                    @endif
                                                </span>
                                            ...
                                            <td class="p-2 align-right">
                                                @if(isset($data->jenis))
                                                    @if($data->jenis == 'masuk')
                                                        <span class="bg-gradient-to-tl from-green-600 to-lime-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Pemasukan</span>
                                                    @elseif($data->jenis == 'keluar')
                                                        <span class="bg-gradient-to-tl from-red-600 to-red-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">Pengeluaran</span>
                                                    @endif
                                                @elseif(isset($data->status))
                                                    @php
                                                        ...
                                                        ][$data->status] ?? 'from-gray-600 to-gray-400';
                                                    @endphp
                                                    <span class="bg-gradient-to-tl {{ $statusClass }} px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">
                                                        {{ ucfirst($data->status) }}
                                                    </span>
                                                @else
                                                    <span class="bg-gradient-to-tl from-gray-600 to-gray-400 px-2.5 py-1.4 text-xs rounded-1.8 text-white font-bold uppercase">N/A</span>
                                                @endif
                                            </td>
                                            <td class="p-2 align-middle">
                                                <div class="flex items-center">
                                                    @if($data->user && $data->user->gambar)
                                                        <img src="{{ asset('storage/users/' . $data->user->gambar) }}" class="w-6 h-6 rounded-full mr-2 object-cover">
                                                    @else
                                                        <div class="w-6 h-6 rounded-full bg-gradient-to-tl from-gray-400 to-gray-600 mr-2 flex items-center justify-center">
                                                            <i class="fas fa-user text-xs text-white"></i>
                                                        </div>
                                                    @endif
                                                    <span class="text-sm font-semibold">{{ $data->user->name ?? 'N/A' }}</span>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="7" class="p-8 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    ...
                                                    <button onclick="resetFilter()" class="inline-block px-6 py-3 text-xs font-bold text-center text-white uppercase bg-gradient-to-tl from-gray-400 to-gray-600 rounded-lg shadow-md hover:scale-102 transition-all">
