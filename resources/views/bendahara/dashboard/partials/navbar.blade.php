...
<nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft rounded-2xl lg:flex-nowrap lg:justify-start"
    ...
    <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        <nav>
            <!-- Breadcrumb Dinamis untuk Bendahara -->
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
                ...
                <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']"
                    aria-current="page">
                    @if(request()->is('bendahara/dashboard'))
                        Dashboard
                    @elseif(request()->is('bendahara/kas*'))
                        Kas Management
                    @elseif(request()->is('bendahara/verifikasi*'))
                        Verifikasi Pengadaan
                    @elseif(request()->is('bendahara/analisis*'))
                        Analisis TOPSIS
                    @elseif(request()->is('bendahara/laporan*'))
                        Laporan
                    @else
                        Dashboard
                    @endif
