...
@section('title', 'Pengajuan Pengadaan - Pengurus')
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
                ...
                <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-5 mb-0 bg-white rounded-t-2xl">
                        <div class="flex flex-wrap items-center justify-between">
                            <div>
                                <h6 class="mb-0 text-lg font-bold text-slate-700">Pengajuan Pengadaan</h6>
                                <p class="mb-0 text-sm leading-normal text-slate-400">Kelola pengajuan barang baru untuk kegiatan gereja</p>
                            </div>
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('pengurus.pengajuan.create') }}" class="inline-block px-6 py-3 text-xs font-bold text-center text-white uppercase bg-gradient-to-tl from-blue-600 to-cyan-400 rounded-lg shadow-md hover:scale-102 transition-all">
        ...
        <div class="flex flex-wrap items-center justify-between px-6 pt-4 pb-2 bg-gray-50 rounded-t-xl">
            <!-- Search di kiri -->
            <form id="searchForm" method="GET" action="{{ route('pengurus.pengajuan.index') }}" class="flex items-center">
            ...
            <!-- Filter di kanan -->
            <form id="filterForm" method="GET" action="{{ route('pengurus.pengajuan.index') }}" class="flex flex-wrap items-end gap-4">
                            ...
                            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xxs font-bold uppercase text-slate-400">Jumlah & Kebutuhan</th>
                                ...
                                <tbody>
                                    @forelse ($pengajuans as $pengajuan)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="p-2">
                                                <p class="mb-0 text-xs font-semibold">{{ $pengajuan->jumlah }} {{ $pengajuan->satuan }}</p>
                                                <p class="mb-0 text-xs text-slate-400">Kebutuhan: {{ $pengajuan->kebutuhan->format('d M Y') }}</p>
                                            </td>
                                            ...
                                            <td class="p-2 text-center">
                                                <div class="flex justify-center items-center space-x-2">
                                                    <a href="{{ route('pengurus.pengajuan.show', $pengajuan->id) }}" class="text-blue-500 hover:text-blue-700 p-2 rounded-lg hover:bg-blue-50 transition-all" title="Detail Pengajuan">
                                                        <i class="fas fa-eye text-sm"></i>
                                                    </a>
                                                    @if ($pengajuan->status == 'pending')
                                                        <a href="{{ route('pengurus.pengajuan.edit', $pengajuan->id) }}" class="text-orange-500 hover:text-orange-700 p-2 rounded-lg hover:bg-orange-50 transition-all" title="Edit Pengajuan">
                                                            <i class="fas fa-edit text-sm"></i>
                                                        </a>
                                                        <form id="deleteForm-{{ $pengajuan->id }}" method="POST" action="{{ route('pengurus.pengajuan.destroy', $pengajuan->id) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" onclick="PengajuanManagement.deletePengajuan({{ $pengajuan->id }}, '{{ addslashes($pengajuan->kode_pengajuan) }}')" ...>                                                                <i class="fas fa-trash text-sm"></i>
                                                            </button>
                                                        </form>
                                                    @endif
                                                </div>
                                        ...
                                        <tr>
                                            <td colspan="4" class="p-8 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    ...
                                                    @else
                                                        <a href="{{ route('pengurus.pengajuan.index') }}" class="inline-block px-6 py-3 text-xs font-bold text-center text-white uppercase bg-gradient-to-tl from-gray-400 to-gray-600 rounded-lg shadow-md hover:scale-102 transition-all">
        ...
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block p-6 my-8 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div class="mt-3 text-center sm:mt-5">
                    <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Hapus Pengajuan Pengadaan</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus pengajuan <span id="item-name" class="font-semibold"></span>? Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
                <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                    <button type="button" onclick="confirmDeleteAction()" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:col-start-2 sm:text-sm">
                        Hapus
                    </button>
...
@push('scripts')
<script>
    // Fungsi ini akan menggunakan fungsi global dari gereja.js
