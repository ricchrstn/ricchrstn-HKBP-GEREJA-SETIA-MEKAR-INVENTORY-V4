...
@section('title', 'Jadwal Audit - Admin')
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
                ...
                <div
                    ...
                    <div class="p-6 pb-5 mb-0 bg-white rounded-t-2xl">
                        <div class="flex flex-wrap items-center justify-between">
                            <div>
                                <h6 class="mb-0 text-lg font-bold text-slate-700">Jadwal Audit</h6>
                                <p class="mb-0 text-sm leading-normal text-slate-400">Kelola jadwal audit inventori barang
                                    gereja</p>
                            </div>
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('admin.jadwal-audit.create') }}"
        ...
        <div class="flex flex-wrap items-center justify-between px-6 pt-4 pb-2 bg-gray-50 rounded-t-xl">
            <!-- Search di kiri -->
            <form id="searchForm" method="GET" action="{{ route('admin.jadwal-audit.index') }}" class="flex items-center">
                <div class="relative flex flex-wrap items-stretch transition-all rounded-lg ease-soft">
                    ...
                    <input type="text" name="search" id="searchInput" value="{{ request('search') }}"
                        ...
                        placeholder="Cari jadwal atau barang..." />
                </div>
            </form>
            <!-- Filter di kanan -->
            <form id="filterForm" method="GET" action="{{ route('admin.jadwal-audit.index') }}"
            ...
            <div class="flex-none w-full max-w-full px-3">
                <div class="relative flex flex-col mb-6 bg-white shadow-soft-xl rounded-2xl">
                    <div class="p-6 pb-0 mb-0 bg-white rounded-t-2xl">
                        <div class="flex flex-wrap items-center justify-between">
                            <div>
                                <h6 class="mb-0 font-bold">Daftar Jadwal Audit</h6>
                                ...
                                <thead>
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xxs font-bold uppercase text-slate-400">Judul &
                                            Barang</th>
                                        <th class="px-6 py-3 text-left text-xxs font-bold uppercase text-slate-400">PIC</th>
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                                ...
                                                <div class="flex px-2 py-1">
                                                    ...
                                                    <div class="flex flex-col justify-center">
                                                        ...
                                                        <p class="mb-0 text-xs text-slate-400">
                                                            {{ $jadwalAudit->barang ? $jadwalAudit->barang->nama : 'Barang tidak ditemukan' }}
                                                        </p>
                                            ...
                                            <td class="p-2">
                                                <p class="mb-0 text-xs font-semibold">
                                                    {{ $jadwalAudit->user ? $jadwalAudit->user->name : 'User tidak ditemukan' }}
                                                </p>
                                                <p class="mb-0 text-xs text-slate-400">
                                                    {{ $jadwalAudit->user ? $jadwalAudit->user->email : '-' }}</p>
                                            </td>
                                            <td class="p-2 text-center">
<span class="text-sm font-semibold text-slate-700">{{ $jadwalAudit->tanggal_audit ? $jadwalAudit->tanggal_audit->format('d M Y') : 'Tanggal tidak tersedia' }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                @php
                                                    $statusClass = [
                                            ...
                                            <td class="p-2 text-center">
                                                <div class="flex justify-center items-center space-x-2">
                                                    <a href="{{ route('admin.jadwal-audit.edit', $jadwalAudit->id) }}"
                                                        class="text-orange-500 hover:text-orange-700 p-2 rounded-lg hover:bg-orange-50 transition-all"
                                                        title="Edit Jadwal">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form id="deleteForm-{{ $jadwalAudit->id }}" method="POST"
                                                        action="{{ route('admin.jadwal-audit.destroy', $jadwalAudit->id) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                                            onclick="confirmDelete({{ $jadwalAudit->id }}, '{{ addslashes($jadwalAudit->judul) }}')"
                                            ...
                                            <td colspan="5" class="p-8 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    ...
                                                    <p class="text-sm text-gray-400 mb-4">
                                                        ...
                                                        @else
                                                            Belum ada jadwal audit yang terdaftar dalam sistem
                                                        @endif
                                                    </p>
                                                    @if (!request()->hasAny(['search', 'status', 'tanggal']))
                                                        <a href="{{ route('admin.jadwal-audit.create') }}"
                        ...
                        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex space-x-2">
                                    {{ $jadwalAudits->withQueryString()->links() }}
                                </div>
                            </div>
                        </div>
                    @endif
